...
const Home = (props) => {
    ...
    const handleMap = () => {
        return dataMobilFilter.map((value, index) => {
            return <CardDefault passenger = {value.passenger} transmission = {value.transmission} year = {value.year} nama={value.name} harga={value.price} foto={value.image} kategori={value.category} fungsiPayment={() => { setParameterPindah(2); dispatch(getDataByID(value.id)) }} />
        })
    }
